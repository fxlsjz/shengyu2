'use strict';
//
app.controller('riskCtrl', function ($rootScope, $scope, $state, $stateParams, $location,$cookies,userInfo,getInterface,pageData,$anchorScroll,$http) {
	
	//风险模式识别 
	var riskPatternA = 'riskPattern';//分析报告
	var riskPatternB = 'click';
	var riskPatternC = 'riskPatternS';
	_hmt.push(['_trackEvent', riskPatternA, riskPatternB, riskPatternC]);
	console.log(riskPatternA);
	...
	$scope.okplay = function(){
		...
		$scope.addcollectPlay = false;
		$scope.addblackPlay = false;
		$scope.detailsPlay = false;
	}
	/*点击详情弹框*/
	$scope.details = function(){
		$scope.detailsPlay = true;
	}
	/*平行坐标echarts*/
	var myChart = echarts.init(document.getElementById('px_echear'));
	var option = '';
	var indices = {
	    name: 0,
	    group: 1,
